...
/// Example: a simple random-roll game that uses VRF randomness.
///
...
impl VrfConsumerContract {
    ...
    /// Submit a VRF randomness request for a random roll.
    ///
    ...
    pub fn random_sample(env: Env, player: Address, context_tag: BytesN<16>) -> u64 {
        player.require_auth();
    ...
    pub fn on_vrf(env: Env, request_id: u64, beta_output: BytesN<32>, _alpha_seed: BytesN<32>) {
        ...
        // Derive a random roll (1-6) from beta_output.
        ...
        // Emit event for the random roll result.
        env.events().publish(
    ...
    /// Query the random roll result for a fulfilled request.
