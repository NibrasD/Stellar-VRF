 ...
 *
 * Flow:
 *   1. Calls random_sample() on the consumer contract (which internally calls
...
// ── 2. Call random_sample() on consumer contract ────────────────────────────────
console.log(`\n[2/4] Calling random_sample() on consumer contract...`);
...
Buffer.from("e2e_random_" + (Date.now() % 100000).toString()).copy(contextTag);
...
console.log(`  ✔ random_sample() succeeded`);
...
const randomRoll = simResult.result?.retval?.value?.();
console.log(`  ✔ CALLBACK CONFIRMED! Consumer contract stored random roll: ${randomRoll}`);
...
╠══════════════════════════════════════════════════════════════════╣
║  random_sample() TX:    ${rollSent.hash}  ║
║  Request ID:        ${String(requestId).padEnd(46)}║
║  Random Roll Result:  ${String(randomRoll).padEnd(46)}║
║                                                                  ║
║  This proves:                                                    ║
